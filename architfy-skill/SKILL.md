...
metadata:
  version: "2.18"
  author: vic-alonso
  based_on: tt-a1i/archify (MIT), Cocoon-AI/architecture-diagram-generator (MIT, v1.0)
---
...
3. Artifact first: the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose. Start with one clear main path, short side branches, sparse labels, and at most 12 primary nodes. Set `meta.quality_profile` to `"showcase"` unless the user explicitly requests a dense `standard` map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, `labelAt`, `labelDx`, or `labelDy` before a diagnostic calls for one; apply at most one diagnosed geometry control per repair. Never fan three or more identically labeled parallel relationships out of one node: collapse that repetition into a single relationship to the boundary or to one representative target. Such a fan is the most common cause of label congestion that no offset can resolve. Plan the vertical budget once, here: the diagram panel plus its conclusion cards must fit a 900px-tall viewport at handoff, so keep the authored Y span and the card count proportional from the start. Re-tuning height after `visual-check` costs a full delivery cycle.
...
5. For a delivered HTML, `deliver` is the final acceptance command:
   ...
   A non-zero exit can never be described as success. A failed delivery preserves any previous output, so do not run `visual-check` on that path: it would inspect the stale last-good artifact, not the failed candidate. If validation fails, change only the diagnosed `subject`, verify `evidence`, choose from `supportedFixes`, and rerun. Continue focused correction while the objective error count reaches a new minimum, within a budget of six repair rounds. Repairing geometry by repositioning the same node or re-nudging the same label across successive rounds means the cause is composition, not offset: once two rounds fail to reach a new minimum, or three rounds still leave more than two diagnostics, spend the next round on the cause instead, by removing a low-value relationship, giving the congested band more spacing, or moving a single node once to a position that makes its routes obvious. When the budget is exhausted, stop and report the unresolved diagnostics truthfully.
