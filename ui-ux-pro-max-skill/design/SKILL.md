...
|------|-----------|---------|
| Brand identity, voice, assets | `uiux-pmx-brand` | Bundled sibling skill |
| Tokens, specs, CSS vars | `uiux-pmx-design-system` | Bundled sibling skill |
| shadcn/ui, Tailwind, code | `uiux-pmx-ui-styling` | Bundled sibling skill |
...
After generation, **ALWAYS** ask user about HTML preview via `AskUserQuestion`. If yes, use the bundled `uiux-pmx-ui-ux-pro-max` skill for the gallery.
...
2. **Research** — Read `references/banner-sizes-and-styles.md` and use the bundled `uiux-pmx-ui-ux-pro-max` skill for style and palette guidance; if browser research is available and permitted, collect 3–5 references
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses the bundled `uiux-pmx-ui-ux-pro-max`, `uiux-pmx-brand`, and `uiux-pmx-design-system` skills; screenshot export runs through Chrome headless, Playwright, or Puppeteer (see the reference).
...
4. **Design** — bundled `uiux-pmx-brand` → `uiux-pmx-design-system` → `uiux-pmx-ui-ux-pro-max` skills; HTML per idea × size
...
1. **Brand** (uiux-pmx-brand skill) → Define colors, typography, voice
2. **Tokens** (uiux-pmx-design-system skill) → Create semantic token layers
3. **Implement** (uiux-pmx-ui-styling skill) → Configure Tailwind, shadcn/ui
...
**Bundled sub-skills:** uiux-pmx-brand, uiux-pmx-design-system, uiux-pmx-ui-styling
**Related Skills:** uiux-pmx-ui-ux-pro-max
